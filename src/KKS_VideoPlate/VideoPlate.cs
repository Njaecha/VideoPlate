...
namespace VideoPlate
{
    ...
    {
        public const string PluginName = "KKS_VideoPlate";
        ...
        internal static void setGuiContent(TreeNodeObject tno = null)
        {
            ...
            if (selectedObject is OCIItem)
            {
                ...
                bool videoPlateSelected = item.objectItem.TryGetComponent<VideoPlatePlayer>(out VideoPlatePlayer vpp);
                if (videoPlateSelected && vpp != null)
                {
        ...
        private void WindowFunction2(int WindowID)
        {
            ...
            if (lastSelectedPlayer != null)
            {
                ...
                lspTime = GUI.HorizontalSlider(new Rect(10, 20, 480, 20), lspTime, 0.0f, (float)vpp.player.length);
                ...
                GUI.Label(new Rect(windowRect2.width-130, 30, 120, 20), $"{TimeSpan.FromSeconds(vpp.player.length)}",textRight);
                ...
                GUI.Label(new Rect(10, 165, 380, 30), $"Resolution: {vpp.player.width}x{vpp.player.height} | Framerate: {vpp.player.frameRate}fps", textCentered);
